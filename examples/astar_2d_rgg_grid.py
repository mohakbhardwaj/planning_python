...
#Step 3: Create a random geometric graph with a uniform grid of nodes
res = 5.0
radius = 10.0
pos = {0: start, 1:goal } #Pos defines the position of the nodes in the graph. we first add the start and the goal
xs = np.linspace(x_lims[0], x_lims[1], (x_lims[1]-x_lims[0])/res)
ys = np.linspace(y_lims[0], y_lims[1], (y_lims[1]-y_lims[0])/res)
i = 2
for j in xs:
  for k in ys:
    pos[i] = (j, k)
...
e.plot_graph(g.graph, pos, arrows=False, node_size=1, edge_color='k')
plt.show()
